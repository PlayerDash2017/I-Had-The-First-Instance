/// @description scrGetKeybind(key,[default])
/// @param key
/// @param [default]
function scrGetKeybind() {
	//gets a string saying what the current keybind is

	var keyIn = argument[0]; //Input key
	...
	switch(keyIn)
	{
	    ...
	    default:
	        var t_char;
	        t_char = chr(keyIn);
	        if (is_string(t_char) && t_char != "" && string_width(string_hash_to_newline(t_char)) > 0){
	            return t_char;
